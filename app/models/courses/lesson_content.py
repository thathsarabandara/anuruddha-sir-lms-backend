...
class LessonContent(db.Model):
    ...
    content_type = db.Column(
        db.Enum("video", "zoom_live", "text", "pdf", "quiz"), nullable=False, index=True
    )
    ...
    video_file_size_bytes = db.Column(db.BigInteger, nullable=True)
    video_quality_available = db.Column(db.String(100), nullable=True)
    ...
    scheduled_duration_minutes = db.Column(db.Integer, nullable=True)
    is_recorded = db.Column(db.Boolean, default=False, nullable=True)
    recording_url = db.Column(db.String(500), nullable=True)

    # Text/PDF Content Fields
    text_content = db.Column(db.Text, nullable=True)
    pdf_file_url = db.Column(db.String(500), nullable=True)
    ...
    progress_records = db.relationship(
        "LessonContentProgress", backref="content", lazy="dynamic", cascade="all, delete-orphan"
    )

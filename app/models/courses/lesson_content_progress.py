...
class LessonContentProgress(db.Model):
    ...
    lesson_id = db.Column(
        db.String(36), db.ForeignKey("course_lessons.lesson_id", ondelete="CASCADE"), nullable=False
    )
    user_id = db.Column(
        db.String(36),
    ...
    video_watched_percentage = db.Column(db.Integer, default=0, nullable=True)
    video_current_position_seconds = db.Column(db.Integer, default=0, nullable=True)
    video_watch_count = db.Column(db.Integer, default=0, nullable=True)
    video_quality_watched = db.Column(db.String(20), nullable=True)
    video_total_watch_time_seconds = db.Column(db.Integer, default=0, nullable=True)

    # Zoom-Specific Fields
    zoom_attended = db.Column(db.Boolean, default=False, nullable=True)
    zoom_joined_at = db.Column(db.DateTime, nullable=True)
    zoom_left_at = db.Column(db.DateTime, nullable=True)
    zoom_attended_duration_minutes = db.Column(db.Integer, nullable=True)
    zoom_device_type = db.Column(db.String(50), nullable=True)

    # PDF-Specific Fields
    pdf_downloaded = db.Column(db.Boolean, default=False, nullable=True)
    pdf_download_count = db.Column(db.Integer, default=0, nullable=True)

    # Quiz-Specific Fields
    quiz_attempted = db.Column(db.Boolean, default=False, nullable=True)
    quiz_score = db.Column(db.Integer, nullable=True)
    quiz_attempt_count = db.Column(db.Integer, default=0, nullable=True)
    quiz_last_attempted = db.Column(db.DateTime, nullable=True)

    __table_args__ = (db.UniqueConstraint("content_id", "user_id", name="unique_user_content"),)
    ...
    def to_dict(self):
        ...
        return {
            ...
            "quiz_last_attempted": (
                self.quiz_last_attempted.isoformat() if self.quiz_last_attempted else None
            ),
        }

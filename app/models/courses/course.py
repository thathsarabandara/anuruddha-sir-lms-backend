...
class Course(db.Model):
    ...
    category_id = db.Column(
        db.String(36), db.ForeignKey("course_categories.category_id"), nullable=True
    )
    ...
    status = db.Column(
        db.Enum("draft", "published", "archived"), default="draft", nullable=False, index=True
    )
    visibility = db.Column(db.Enum("public", "private"), default="public", nullable=False)
    course_type = db.Column(
        db.Enum("monthly", "paper", "quiz", "special"), default="monthly", index=True
    )
    ...
    sections = db.relationship(
        "CourseSection", backref="course", lazy="dynamic", cascade="all, delete-orphan"
    )
    lessons = db.relationship(
        "CourseLesson", backref="course", lazy="dynamic", cascade="all, delete-orphan"
    )
    enrollments = db.relationship(
        "CourseEnrollment", backref="course", lazy="dynamic", cascade="all, delete-orphan"
    )
    reviews = db.relationship(
        "CourseReview", backref="course", lazy="dynamic", cascade="all, delete-orphan"
    )
    activity_logs = db.relationship(
        "CourseActivityLog", backref="course", lazy="dynamic", cascade="all, delete-orphan"
    )
    enrollment_keys = db.relationship(
        "CourseEnrollmentKey", backref="course", lazy="dynamic", cascade="all, delete-orphan"
    )
    status_audits = db.relationship(
        "CourseStatusAudit", backref="course", lazy="dynamic", cascade="all, delete-orphan"
    )
    contents = db.relationship(
        "LessonContent", backref="course", lazy="dynamic", cascade="all, delete-orphan"
    )
    quiz_courses = db.relationship(
        "QuizCourse", backref="course", lazy="dynamic", cascade="all, delete-orphan"
    )

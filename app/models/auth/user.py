...
class User(db.Model):
    ...
    login_history = db.relationship(
        "LoginHistory", backref="user", lazy=True, cascade="all, delete-orphan"
    )
    login_failures = db.relationship(
        "LoginFailure", backref="user", lazy=True, cascade="all, delete-orphan"
    )
    otp_requests = db.relationship(
        "OTPRequest", backref="user", lazy=True, cascade="all, delete-orphan"
    )

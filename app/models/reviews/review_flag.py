...
class ReviewFlag(db.Model):
    ...
    flagged_by = db.Column(db.String(36), db.ForeignKey("users.user_id"), nullable=False)

    # Flag Information
    reason = db.Column(db.Enum("spam", "harassment", "misinformation", "other"), nullable=True)
    description = db.Column(db.Text, nullable=True)

    # Status
    status = db.Column(
        db.Enum("pending", "reviewed", "resolved"), default="pending", nullable=False, index=True
    )
